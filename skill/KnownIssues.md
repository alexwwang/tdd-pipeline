...
Last updated: Signal Purity compression (post-R20)

## P-tier (Proposals / Improvements)

### KI-01 | P | L116-122 | Merge informational Phase 7 findings categories
- **Raised-in**: R18
...
### KI-02 | P | L13 | Unify zero-AC + Category 3 abbreviated report path
- **Raised-in**: R19
- **Re-raised-in**: R20
...
### KI-03 | P | L17 | Replace "Rollback Guidance" reference with "C/H classification reference"
- **Raised-in**: R16, R17, R19, R20
- **Description**: L17 references Phase 7's "Rollback Guidance section" as a load dependency. Phase 8 only needs C/H classification definitions, not rollback targets. Decoupling would strengthen scope boundary.
...
### KI-04 | P | L161 | Secondary AC release-ready formula not explicit
- **Raised-in**: R17
...
- **Plan**: Add a parenthetical note after L161 clarifying the set relationships.

### KI-05 | P | L238 | Part 4 correction cycles — no cascade escalation
- **Raised-in**: R19, R20
...
### KI-06 | L | L17 | Execution paragraph too long
...
- **Why deferred**: Structural improvement only; no logical or scope defect. **Partially resolved** by Signal Purity compression (L17 is shorter but still a single paragraph).
- **Plan**: Break into titled sub-paragraphs in next edit pass.

### KI-07 | L | L53 | Status symbols paragraph dense (~200 words)
...
- **Why deferred**: Reference paragraph functioning correctly; table format is readability improvement. **Resolved** by Signal Purity compression (L53 is now a one-liner).

### KI-08 | L | L116-122 | Phase 7 findings table mixed row types
- **Raised-in**: R19, R20
...
### KI-09 | L | L53 vs L80 | Bad Requirement dual symbol (❓→❌)
- **Raised-in**: R20
...
- **Why deferred**: The mapping is documented at L53. No functional impact.
- **Plan**: Consider consolidating to ❌ with a note in next edit pass.

## I-tier (Informational — no action)

### KI-10 | I | L30 | Phase 6 re-run metadata contract wording
- **Raised-in**: R19, R20
- **Description**: L30 uses "must include" when specifying Phase 6 re-run metadata. Technically prescribes upstream format. Precision consensus: this is an interface contract (Phase 8's input requirement), not a quality standard. Wording could be softened.
...
### KI-11 | I | L91 | Escalated Unknown re-invocation model implicit
- **Raised-in**: R18, R19, R20
...
- **Plan**: No action needed; note at L91 is sufficient.

### KI-12 | I | L254 | Post-verification report modification without re-verification
- **Raised-in**: R17, R18, R19, R20
